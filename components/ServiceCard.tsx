    ...
    >
      ...
      <div className="absolute inset-0 p-8 md:p-10 flex flex-col justify-between pointer-events-none">
        <div className="flex justify-between items-start">
          <span className="text-xs font-mono font-medium tracking-wider border border-brand-caramel/40 bg-white/80 text-brand-ink px-4 py-2 rounded-full backdrop-blur-md">{service.category}</span>
          <motion.div variants={{ rest: { opacity: 0, x: 20, y: -20 }, hover: { opacity: 1, x: 0, y: 0 } }} className="bg-brand-bronze text-white rounded-full p-3 will-change-transform shadow-lg">
        ...
        <div>
          ...
          <div className="overflow-hidden">
            <motion.h3 className="font-heading text-2xl md:text-3xl font-light uppercase text-brand-ink tracking-tight will-change-transform" variants={{ rest: { y: 0 }, hover: { y: -4 } }} transition={{ duration: 0.4 }}>
              {service.name}
            </motion.h3>
          </div>
          <motion.p className="text-xs font-semibold uppercase tracking-widest text-brand-bronze mt-3 will-change-transform" variants={{ rest: { opacity: 0, y: 10 }, hover: { opacity: 1, y: 0 } }} transition={{ duration: 0.4, delay: 0.1 }}>
            {t('viewDetails')}
          </motion.p>
        </div>
      </div>
