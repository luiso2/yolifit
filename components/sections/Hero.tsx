    ...
    <header className="relative min-h-[100svh] flex flex-col items-center justify-center overflow-hidden px-4 md:px-6 pt-28 pb-16 md:pt-24 md:pb-12">
      ...
      <motion.div style={{ y, opacity }} className="z-10 w-full max-w-6xl">
        <div className="flex flex-col items-center md:items-start text-center md:text-left md:max-w-sm">
          ...
          <div className="relative w-full flex flex-col items-center md:items-start">
            <h1 className="text-center md:text-left leading-none">
              ...
              <span className="block text-[13vw] md:text-[4.75rem] lg:text-[5.5rem] font-heading font-medium leading-[0.95]">
                <span className="text-brand-bronze">Yoly</span> <span className="font-light text-gray-500">Studio</span>
              </span>
            </h1>
            <motion.div
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9, duration: 1 }} className="mt-8 flex flex-col sm:flex-row gap-4">
            <button onClick={() => scrollToSection('servicios')} className="px-8 py-3.5 bg-brand-bronze text-white hover:bg-brand-brown transition-all text-xs font-semibold tracking-widest uppercase font-heading rounded-none shadow-md" data-hover="true">
              {t('ctaTreatments')}
            </button>
            <button onClick={() => scrollToSection('experiencia')} className="px-8 py-3.5 border border-brand-bronze/30 text-brand-bronze hover:bg-white/40 transition-all text-xs font-semibold tracking-widest uppercase font-heading rounded-none" data-hover="true">
              {t('ctaAbout')}
            </button>
          </motion.div>
        </div>
