    ...
    <section id="instagram" className="relative z-10 py-20 md:py-32 bg-white/30 border-t border-b border-black/[0.04]">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="text-center mb-12 md:mb-16">
          ...
          <p className="text-gray-600 text-sm mt-3 max-w-lg mx-auto font-light">{t('description')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          <div className="lg:col-span-7 flex flex-col justify-between bg-white/70 border border-brand-bronze/10 rounded-3xl p-6 md:p-10 shadow-sm backdrop-blur-md">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-xs font-mono bg-brand-bronze/35 px-3 py-1 rounded-full text-brand-bronze font-semibold border border-brand-bronze/10">{t('featured')}</span>
            </div>

            <div className="flex-1 flex flex-col justify-center">
              <blockquote className="text-lg md:text-xl font-light italic leading-relaxed text-gray-800 border-l-2 border-brand-caramel pl-6 py-2 my-4">&quot;{t('quote')}&quot;</blockquote>
              <p className="text-xs md:text-sm text-gray-600 leading-relaxed mt-3 font-light">{t('postBody')}</p>
            </div>

            <div className="mt-8 flex flex-wrap gap-4 items-center justify-between border-t border-black/[0.05] pt-6">
              <div className="flex gap-6 text-xs font-mono text-gray-500">
                <span className="flex items-center gap-1.5"><Heart className="w-4 h-4 text-rose-500" /> {t('likes', { count: '1,580' })}</span>
                <span className="flex items-center gap-1.5"><Instagram className="w-4 h-4 text-brand-bronze" /> {t('comments', { count: '92' })}</span>
              </div>
              <button onClick={() => scrollToSection('servicios')} className="px-6 py-3 border border-brand-bronze/20 hover:bg-brand-bronze hover:text-white transition-all text-xs font-semibold tracking-widest uppercase font-heading rounded-none text-brand-bronze bg-transparent" data-hover="true">
                {t('cta')}
              </button>
            </div>
          </div>
